...
from settings import DATASET, GRAPH, SPARQL_ENDPOINT, UPDATE_ENDPOINT
...
with open('gutenberg_2020_bookshelfs.json') as f:
	bookshelves_to_link = json.load(f)

sparql = SPARQLWrapper('/'.join([DATASET, SPARQL_ENDPOINT]), updateEndpoint='/'.join([DATASET, UPDATE_ENDPOINT]))
...
"""
        ...
        for bind in results['results']['bindings']:
                if RDF.value == URIRef(bind['p']['value']):
                        ...
                        slug = ''.join(slug)
                        slug = unidecode.unidecode(urllib.parse.quote(slug, safe=' ')).lower().replace(' ', '_')
...
INSERT {{
        ...
        <{shelf}> dct:title "{shelfname}"
         ; rdfs:seeAlso <https://www.gutenberg.org{bookshelves_to_link[shelfname]}>
         ; a ?t
}}
WHERE {{
        ?x pgterms:bookshelf ?b . 
